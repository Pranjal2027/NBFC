		...
		String token =TokenProvider.getInstance().getToken();
		 ...
		 request.put("seller_id", SELLERID);
 		 ...
 		 given().
		   ...
		   when().
		      post(CONTEXTPATH + "nbfcs/unmap-nbfc/" +NBFCID).
		   then().
		   log().all().statusCode(200);
		   
	 	}
    @Test
   	public void nbfc_ummap_replace_seller_id_to_buyer_id_vice_versa()
   	{
   		JSONObject request=new JSONObject();
   		String token =TokenProvider.getInstance().getToken();
		 ...
		 request.put("seller_id", BUYERID);
    		 ...
    		 given().
   		   contentType("application/json").
   		      ...
   		      header("authorization"," Bearer " + token).
   		   when().
   		      post(CONTEXTPATH + "nbfcs/unmap-nbfc/" +NBFCID).//nbfcid/createdby
   		   then().
   		   log().all().
   		    statusCode(400);
   	 	}
